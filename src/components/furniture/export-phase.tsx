...
export default function ExportPhase() {
  ...
  const handleDownloadPNG = useCallback(async () => {
    try {
      ...
      if (!canvas) { toast.error("Error al generar imagen"); return; }
      const link = document.createElement("a");
      link.download = "ficha-editada.png";
      link.href = canvas.toDataURL("image/png");
      link.click();
      toast.success("PNG descargado");
    } catch { toast.error("Error al generar PNG"); }
  }, [composeCanvas]);
  ...
  const handleDownloadJPG = useCallback(async () => {
    ...
    } catch { toast.error("Error al generar JPG"); }
  }, [composeCanvas]);
    ...
    <div className="min-h-screen flex flex-col">
      <header className="border-b border-border/50 bg-background/80 backdrop-blur-sm sticky top-0 z-20 print:hidden">
        <div className="px-4 sm:px-6 h-14 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Button variant="ghost" size="sm" onClick={() => setPhase("editing")} className="gap-1.5">
              <ArrowLeft className="h-4 w-4" /> Volver al Editor
            </Button>
            <Separator orientation="vertical" className="h-6" />
            <h1 className="text-lg font-bold">
              <span className="text-primary">Mobili</span>{" "}
              <span className="text-muted-foreground font-normal text-sm">— Exportar</span>
            </h1>
          </div>
        </div>
      </header>
      ...
      <main className="flex-1 flex flex-col lg:flex-row print:block">
        ...
        <div className="w-full lg:w-80 xl:w-96 border-t lg:border-t-0 lg:border-l border-border/50 p-4 sm:p-6 space-y-4 print:hidden">
          ...
          <p className="text-sm text-muted-foreground">Descarga tu ficha con los textos editados.</p>
          <Card>
            <CardContent className="p-4 space-y-3">
              <Button onClick={handleDownloadPNG} className="w-full gap-2" size="lg">
                <FileImage className="h-5 w-5" /> Descargar PNG
              </Button>
              <Button onClick={handleDownloadJPG} variant="outline" className="w-full gap-2" size="lg">
                <Download className="h-5 w-5" /> Descargar JPG
              </Button>
              <Button onClick={() => window.print()} variant="outline" className="w-full gap-2" size="lg">
          ...
          {detectionResult && (
            <div className="text-xs text-muted-foreground space-y-1">
              <p><strong>Campos:</strong> {editedRegions.length}</p>
              <p><strong>Resolución:</strong> {detectionResult.imageWidth}×{detectionResult.imageHeight}px</p>
            </div>
          )}
        </div>
      </main>
